...
export default function Timeline({ date, appointments = [], doctors = [], onDateChange }) {
  ...
  return (
    <div className="flex h-full w-full flex-col">
      {/* Antetul de sus (Data și Navigare) */}
      <div className="flex items-center justify-between border-b bg-white px-4 z-20" style={{ height: HEADER_HEIGHT }}>
        <div className="flex items-center gap-2">
          <label className="text-sm font-medium text-slate-700">Selectează data:</label>
          <input
            type="date"
            value={activeDate}
        ...
        <div className="text-xs text-slate-500">Program: {formatHour(START_HOUR)} – {formatHour(END_HOUR)}</div>
      </div>

      <div className="relative flex-1 overflow-hidden">
        <div ref={containerRef} className="absolute inset-0 overflow-auto cursor-grab">
                    ...
                    {(doctorIdToAppts.get(doc.id) || []).map((a, i) => {
                      ...
                      return (
                        <div
                          ...
                          className="absolute left-2 right-2 rounded-md border bg-white shadow-card transition-shadow hover:shadow-sm hover:ring-1 hover:ring-slate-300 z-10" // Adaugă un z-index pentru siguranță
                          style={{ top, height }}
                        >
                          <div className="grid h-full grid-rows-3 px-2 py-1 text-xs">
                            <div className="truncate font-medium text-slate-900">{a?.patient?.name || 'Pacient'}</div>
                            <div className="truncate text-slate-600">{a?.doctor?.name || 'Medic'}</div>
                            <div className="flex items-center justify-between text-slate-600">
                              <span className="truncate">{a?.service?.name || a?.treatmentType}</span>
        ...
        <SheetContent side="right">
          ...
          {selectedAppt && (
            <div className="space-y-2 text-sm">
              <div><span className="text-slate-500">Pacient:</span> {selectedAppt?.patient?.name}</div>
              <div><span className="text-slate-500">Medic:</span> {selectedAppt?.doctor?.name}</div>
              <div><span className="text-slate-500">Tratament:</span> {selectedAppt?.service?.name || selectedAppt?.treatmentType}</div>
              <div><span className="text-slate-500">Data:</span> {selectedAppt?.date || selectedAppt?.appointmentDate}</div>
              <div><span className="text-slate-500">Ora:</span> {selectedAppt?.time || selectedAppt?.startTime}</div>
            </div>
          )}
        </SheetContent>
      </Sheet>
